...
namespace Services
{
         ...
         where TEntity : BaseEntity<TKey>, new()
    {
        ...
        public ServiceRepository(IMapper mapper, IBaseRepository<TEntity> repository)
        {
            this.mapper = mapper;
        ...
        public virtual async Task<PagingDto<TSelectDto>> Select(int pageNumber, int pageSize, CancellationToken cancellationToken = default)
        {
            ...
            {
                Data = await repository.TableNoTracking
                .Skip((pageNumber - 1) * pageSize)
                .Take(pageSize).ProjectTo<TSelectDto>(mapper.ConfigurationProvider)
                .ToListAsync(cancellationToken),
                TotalRowCount = await repository.TableNoTracking.CountAsync(cancellationToken)
            };
    ...
    {
        public ServiceRepository(IMapper mapper, IBaseRepository<TEntity> repository) : base(mapper, repository)
        {
        }
    }
