...
{
    public class BaseRepository<TEntity> : IBaseRepository<TEntity>, IScopedDependency
        where TEntity : class, IEntity
    {
        ...
        public virtual async Task UpdateAsync(TEntity entity, CancellationToken cancellationToken, bool saveNow = true)
        {
            ...
            if (primaryKey.PropertyInfo.PropertyType == typeof(Guid))
            {
                localEntity = DbContext.Set<TEntity>().Local.FirstOrDefault(p => new Guid(p.GetType().GetProperty(primaryKey.Name).GetValue(p).ToString())
                    .Equals(new Guid(entity.GetType().GetProperty(primaryKey.Name).GetValue(entity).ToString())));
            }
            else
            {
                localEntity = DbContext.Set<TEntity>().Local
                  .FirstOrDefault(p => Convert.ChangeType(p.GetType().GetProperty(primaryKey.Name).GetValue(p), primaryKey.PropertyInfo.PropertyType, CultureInfo.InvariantCulture)
                   .Equals(Convert.ChangeType(entity.GetType().GetProperty(primaryKey.Name).GetValue(entity), primaryKey.PropertyInfo.PropertyType, CultureInfo.InvariantCulture)));
            }
            if (localEntity != null)
        ...
        public async Task<ColumnType> GetColumnValueAsync<ColumnType>(Expression<Func<TEntity, bool>> predicate, Expression<Func<TEntity, ColumnType>> selector, CancellationToken cancellationToken)
        {
            return await TableNoTracking.Where(predicate).Select(selector).FirstOrDefaultAsync(cancellationToken).ConfigureAwait(false);
        }
        ...
        public async Task<ColumnType> GetColumnValueAsync<ColumnType>(Expression<Func<TEntity, bool>> predicate, CancellationToken cancellationToken, string columnName = "Id")
        {
            ...
            if (entityType.GetProperty(columnName) != null)
            {
                ...
                if (entity != null && entityType.GetProperties().Any(p => p.Name == columnName))
                {
                    ...
                    if (propertyInfo is { }) return (ColumnType)propertyInfo.GetValue(entity, null);
                }
            }
            return default;
        }

        public ColumnType GetColumnValue<ColumnType>(Expression<Func<TEntity, bool>> predicate , Expression<Func<TEntity, ColumnType>> selector)
        {
            return TableNoTracking.Where(predicate).Select(selector).FirstOrDefault();
        }
                                                       ...
                                                       params Expression<Func<TEntity, object>>[] navigationPropertyPaths)
        {
            ...
            foreach (var navigationProperty in navigationPropertyPaths)
                query = asNoTracking ? query.Include(navigationProperty).AsNoTracking() : query.Include(navigationProperty);
