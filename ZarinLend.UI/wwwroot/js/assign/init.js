$(function () {
  ...
  $("#btnSave").on("click", function () {
    ...
    if (accountManagerData == 8 || accountManagerData == 30) {
    ...
    $.post(postAssignment, dataRequestAssign, function (data, status) {
      if ("success") {
        $.each(data.entity, function (i, item) {
          ...
          col1.appendChild(document.createTextNode(item.customerNo));
          tr.appendChild(col1);

          let col2 = document.createElement("td");
          col2.appendChild(document.createTextNode(item.customerFullName));
          tr.appendChild(col2);

          let col3 = document.createElement("td");
          col3.appendChild(document.createTextNode(item.personnelId));
          tr.appendChild(col3);

          let col4 = document.createElement("td");
          col4.appendChild(document.createTextNode(item.employeefullName));
          tr.appendChild(col4);

          let col5 = document.createElement("td");
          col5.appendChild(document.createTextNode(item.jobTitle));
          tr.appendChild(col5);

          let col6 = document.createElement("td");
          col6.appendChild(document.createTextNode(item.departmentCode));
          tr.appendChild(col6);

          let col7 = document.createElement("td");
          col7.appendChild(document.createTextNode(item.regionCode));
          tr.appendChild(col7);

          let col8 = document.createElement("td");
          col8.appendChild(document.createTextNode(item.areaId));
          tr.appendChild(col8);

          let col9 = document.createElement("td");
          col9.appendChild(document.createTextNode(item.branchId));
          tr.appendChild(col9);

          selectedCustomers.appendChild(tr);
        });
...
/*
		@Ids nvarchar(max)			,
		@Status int					, --2	دارای مدیر حساب
									  --3	معلق شده
		@ByPersonnelId bigint		,
		@ToRole int					,
		@ToPersonnelId bigint		
*/
