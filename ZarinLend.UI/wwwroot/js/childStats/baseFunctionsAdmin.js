...
function getBranchInfoByEmployee(action) {
  ...
  $.getJSON(getBranchInfoByEmployeeUrl, dataGetChildStatsA, function (data) {
    ...
    data.items.forEach(function (item, index) {
      let rowTemplate = ` <tr>
                        <th>
                        <a href="/Admin/Profile/Index/${item.personnelId}"  target="_blank">
...
$("#btnFilter").on("click", function () {
  ...
  if (!branchEmployee.classList.contains("hidden")) {
    branchEmployee.classList.remove("visible");
    branchEmployee.classList.add("hidden");
  }

  if (!branchCustomers.classList.contains("hidden")) {
    branchCustomers.classList.remove("visible");
    branchCustomers.classList.add("hidden");
  }
  ...
  $.getJSON(
    ...
    function (data) {
      ...
      data.items.forEach(function (item) {
        let rowTemplate = ` <tr>
                          <th>
                              <a href="/Admin/CustomerInfo/${item.customerNo}" target="_blank">
